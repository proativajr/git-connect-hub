...
import { Plus, X, Clipboard, LayoutGrid, LayoutList, CalendarDays, Trash2, GripVertical } from "lucide-react";
...
const MondayBoardSupabase = ({ boardId, title }: { boardId: string; title: string }) => {
  ...
  if (loading) {
    return <div className="space-y-4 transition-page"><div className="h-8 w-48 bg-muted rounded animate-pulse" /><div className="grid grid-cols-4 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-64 bg-card rounded-xl animate-pulse border border-border" />)}</div></div>;
  }
    ...
    <div className="space-y-5 transition-page">
              ...
              <div key={col.id} className="min-w-[260px] flex-shrink-0 rounded-xl bg-muted/50 p-3 space-y-2"
                ...
                {colItems.map(item => (
                  <div key={item.id} draggable onDragStart={() => handleDragStart(item.id)}
                    className={`rounded-lg bg-card border border-border p-3 cursor-grab hover:-translate-y-px hover:shadow-md transition-all duration-150 group ${dragItem === item.id ? "opacity-60 border-accent border-2" : ""}`}>
                    ...
                    <button onClick={() => handleDeleteItem(item.id)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition-opacity">
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
                ))}
                <button onClick={() => setShowNewPanel(true)} className="w-full text-xs text-muted-foreground hover:text-foreground py-2 transition-colors">
                  + Adicionar Item
                </button>
              </div>
            );
              ...
              {items.map((item, i) => (
                <tr key={item.id} className={`border-b border-border/50 hover:bg-muted/30 ${i % 2 ? "bg-background-secondary" : ""}`}>
                  <td className="px-4 py-3">
                    <input value={item.title} onChange={e => handleInlineEdit(item.id, "title", e.target.value)}
                  ...
                  <td className="px-4 py-3">
                    <input type="date" value={item.due_date || ""} onChange={e => handleInlineEdit(item.id, "due_date", e.target.value)}
                      className="bg-transparent text-xs text-muted-foreground outline-none" />
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleDeleteItem(item.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showNewPanel && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-[420px] max-w-full h-full bg-card border-l border-border p-6 space-y-5 overflow-y-auto shadow-xl animate-fade-in" style={{ transform: "translateX(0)" }}>
