...
/* Fluted Doric pillar SVG — classical Greek style with vertical fluting */
...
const Culture = () => {
  ...
  // Scroll to governance section if navigated with hash
  useEffect(() => {
    ...
    <div className="relative -m-8" style={{ overflow: "hidden", minHeight: "calc(100vh - 4rem)" }}>
        ...
        {animPhase === "detail" && selectedPillar && (
          <div className="animate-fade-in p-8 pt-12 min-h-[calc(100vh-4rem)]">
            ...
            <p className="text-white/50 text-lg mb-10">
              Cada dimensão fortalece o coletivo.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl">
              {selectedPillar.characteristics.map((item, i) => (
                <div
                  key={i}
                  className="flex items-start gap-4 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/10 animate-fade-in"
          ...
          <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] px-8">
            ...
            <div className={`text-center mb-12 transition-all duration-500 ${animPhase === "rising" ? "opacity-0 -translate-y-8" : "opacity-100"}`}>
              <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight mb-3">
                Identidade do Cardume
              </h1>
              <p className="text-white/40 text-lg">
                Os pilares sustentam o Cardume. Escolha um pilar para explorar.
              </p>
            </div>

            {/* Entablature */}
            <div className={`w-full max-w-3xl transition-all duration-700 ${animPhase === "rising" ? "opacity-0 -translate-y-16" : "opacity-100"}`}>
              <div className="h-3 bg-white/90 rounded-t-sm" />
              ...
              {pillars.map((pillar, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex flex-col items-center gap-3 transition-all ease-in-out ${
                      isRising
                        ? "-translate-y-[200%] opacity-0 duration-[800ms]"
                        : isOther
                        ? "opacity-20 blur-[2px] scale-95 duration-500"
                  ...
                  >
                    <div className="w-16 h-44 md:w-20 md:h-56 group-hover:-translate-y-3 group-hover:drop-shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-all duration-300">
                      <DoricPillar glowing={false} />
                    </div>
                    <span className="text-xs md:text-sm font-bold uppercase tracking-[0.25em] text-white/80 group-hover:text-white transition-colors">
                      {pillar.name}
                    </span>
                  </button>
                );
            ...
            <div className={`w-full max-w-3xl -mt-px transition-all duration-700 ${animPhase === "rising" ? "opacity-0 translate-y-8" : "opacity-100"}`}>
              <div className="h-1 bg-white/30" />
              <div className="h-2 bg-white/60" />
              <div className="h-4 bg-white/90 rounded-b-sm" />
            </div>

            {/* Governance section below pillars */}
            <div
              ref={governanceRef}
              id="governanca"
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="rounded-lg bg-white/10 p-2.5">
                        <doc.icon className="h-5 w-5 text-white/70" />
                      </div>
                    </div>
      ...
      <Dialog open={!!selectedDoc} onOpenChange={() => setSelectedDoc(null)}>
        <DialogContent className="bg-[#0a2540] border-white/10 text-white">
          ...
          <div className="pt-2">
            <p className="text-sm text-white/60 leading-relaxed">{selectedDoc?.detail}</p>
          </div>
        </DialogContent>
      </Dialog>
    </div>
